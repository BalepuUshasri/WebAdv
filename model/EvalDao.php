...
class EvalDao {
  ...
  /**
   * 
   * @param int $trainer_id id of a trainee
   * @return $list return a list of evaluations for a trainer
   */
  public static function getEvaluationsTrainer($author_id) {
    $db = DB::getConnection();
    $sql = "select evaluation_id,author_id,usergroup.group_id,usergroup.name,title,evaluation.scheduled_at,evaluation.ending_at,evaluation.corrected_at,timediff(ending_at,scheduled_at) as time_diff from sql_quiz natural join evaluation natural join usergroup where author_id=:author_id";
    $stmt = $db->prepare($sql);
   ...
   * @return array $filtred  list of filtred evaluations
   */
  public static function getFilteredTrainer($evaluations) {
    $coming = array();
    $finished = array();
    $corrected = array();
    $filtred = array();
