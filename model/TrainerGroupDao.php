...
class TrainerGroupDao {
  ...
  public static function getMembers($group_id) {
    $db = DB::getConnection();
    $sql = "SELECT group_id,person.person_id as trainee_id,name,first_name,group_member.validated_at as member_validated_at FROM group_member JOIN person on group_member.person_id=person.person_id where group_id=:group_id and is_trainer=false";
    $stmt = $db->prepare($sql);
    $stmt->bindValue(":group_id", $group_id);
    $stmt->execute();
    $db = null;
...
/**
   * 
   * @param type $group_id id of a group
   * @param type $trainee_id id of trainee
   * @return array $ok  return 1  for insert succefull
   */
  public static function validateCandidate($group_id, $trainee_id) {
